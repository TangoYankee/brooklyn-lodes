...
```{r}
subway <- readr::read_csv('data/nta-subway-times.csv')
```
...
## Transformed
```{r}
t_subway_frame <- dplyr::data_frame(from = subway$nta_one, to = subway$nta_two, weight = (1 / (subway$seconds_in_transit ^ (1/8))))

t_subway_graph <- igraph::graph.data.frame(t_subway_frame)
t_subway_matrix <- igraph::as_adjacency_matrix(t_subway_graph, attr = "weight")
t_subway_weights <- t_subway_matrix %>%
  spdep::mat2listw()
```

```{r}
t_subway_global_morans <- spdep::moran.test(
  log(bk_dest_borders$S000),
  t_subway_weights,
  zero.policy = TRUE,
)
print(t_subway_global_morans)
```
...
t_subway_bk_lisa <- spdep::localmoran(
...
bk_lisa_classes <- t_subway_bk_lisa %>%
  tibble::as_tibble() %>%
  magrittr::set_colnames(
    c("Ii","E.Ii","Var.Ii","Z.Ii","Pr(z > 0)")
  ) %>%
...
## Note unnconnected 
```{r}
lines <- readr::read_csv('data/nta-subway-lines.csv')
summary(lines$line_count)

subway_connected <- subway %>%
  dplyr::left_join(lines) %>%
  dplyr::mutate(
    connected_seconds = ifelse(line_count > 0, seconds_in_transit, Inf),
    i_connected_seconds = 1 / connected_seconds^(1 / 8),
    log_S000 = S000
    )
```

```{r}
subway_connected_frame <- dplyr::data_frame(
  from = subway_connected$nta_one,
  to = subway_connected$nta_two,
  weight = subway_connected$i_connected_seconds,
)

subway_connected_graph <- igraph::graph.data.frame(subway_connected_frame)
subway_connected_matrix <- igraph::as_adjacency_matrix(
  subway_connected_graph,
  attr = "weight"
)

subway_connected_weights <- subway_connected_matrix %>%
  spdep::mat2listw()
```

```{r}
subway_connected_global_morans <- spdep::moran.test(
  t_bk_dest_borders$log_S000,
  subway_connected_weights,
  zero.policy = TRUE,
)
print(subway_connected_global_morans)
```

```{r}
subway_connected_lisa <- spdep::localmoran(
  t_bk_dest_borders$log_S000,
  subway_connected_weights,
  zero.policy = TRUE,
  na.action = na.omit
...
subway_connected_classes <- subway_connected_lisa %>%
  tibble::as.tibble() %>%
  magrittr::set_colnames(
    c("Ii","E.Ii","Var.Ii","Z.Ii","Pr(z > 0)")
  ) %>%
...
t_bk_dest_borders$coType <- subway_connected_classes$coType %>%
