# Auto-correlation for Walking Contiguity at NTA level
...
```{r}
walking_frame <- dplyr::data_frame(
  from = walking$nta_one,
  to = walking$nta_two,
  weight = 1 / walking$seconds_of_walking^(1/8)
)

walking_graph <- igraph::graph.data.frame(walking_frame)
walking_matrix <- igraph::as_adjacency_matrix(walking_graph, attr = "weight")

walking_weights <- walking_matrix %>%
  spdep::mat2listw()
```
...
```{r}
walking_bk_lisa <- spdep::localmoran(
  bk_dest_borders$log_S000,
  walking_weights,
  zero.policy = TRUE,
  na.action = na.omit,
...
sig_lev <- 0.05
avg_jobs <- mean(bk_dest_borders$log_S000)

bk_lisa_classes <- walking_bk_lisa %>%
  tibble::as_tibble() %>%
  ...
  dplyr::mutate(
    coType = dplyr::case_when(
      ...
      `Pr(z > 0)` <= 0.05 & Ii >= 0 & bk_dest_borders$log_S000 >= avg_jobs ~ "HH",
      `Pr(z > 0)` <= 0.05 & Ii >= 0 & bk_dest_borders$log_S000 <  avg_jobs ~ "LL",
      `Pr(z > 0)` <= 0.05 & Ii < 0 & bk_dest_borders$log_S000 >= avg_jobs ~ "HL",
      `Pr(z > 0)` <= 0.05 & Ii < 0 & bk_dest_borders$log_S000 < avg_jobs ~ "LH"
    )
  )
...
## Transformed
```{r}
t_driving_frame <- dplyr::data_frame(from = walking$nta_one, to = walking$nta_two, weight = (1 / (walking$seconds_in_traffic ^ (1/8))))
