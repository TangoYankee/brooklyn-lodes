...
```{r, message=FALSE}
## bk_dest : in
## bk_tract_neighbors : in
## bk_tract_neighbors_map: intermediar
## bk_dest_neighbors: out
## TODO: check on list for nb list. Was cartodbid
...
for (i in 1:length(bk_tract_neighbors_map)) {
  ...
  for(j in 1:length(neighbors)) {
    ## neighbors[j] <- bk_tract_neighbors[[neighbor_positions[j]]]
...
bk_od_undirected_neighbors <- bk_od_undirected %>%
  ...
  dplyr::mutate(are_neighbors = ifelse(census_tract_b %in% unlist(neighbors), 1, 0))
